...
void test_compile_literal_expression() {
    ...
    assert(result->constants[3].type == VAL_INT);
    assert(result->constants[3].int_val == 42);
...
void test_compile_bool_literal() {
    ...
    assert(result->code_array.bytecodes != NULL);
    assert(result->constants_count >= 1);
    assert(result->constants[1].type == VAL_BOOL);
    assert(result->constants[1].bool_val == true);
...
void test_compile_assignment_statement_local() {
    ...
    bytecode_array_print(&result->constants[3].code_val->code);
    for (uint32_t i = 0; i < result->constants[3].code_val->code.count; i++) {
        bytecode bc = result->constants[3].code_val->code.bytecodes[i];
        if (bc.op_code == STORE_FAST) has_store_fast = true;
    }
